...
const getDistFuseBoxConfig = (
  conf,
  quantum = {},
  fuseBoxPackageId = "fuse-box",
) => {
...
gulp.task("commit-release", function(done) {
  ...
  exec(
    `git add .; git commit -m "chore(publish): Release ${
      json.version
    }" -a; git tag v${json.version}; git push origin master --tags`,
    (error, stdout, stderr) => {
      if (error) {
...
fuse.run();`;
    ...
    fs.writeFileSync(
      path.join(playgroundFolder, "src/index.ts"),
