const Client = require("fuse-box-websocket").SocketClient,
  bundleErrors = {},
...
function displayBundleErrors() {
  const errorMessages = Object.keys(bundleErrors).reduce(
      (allMessages, bundleName) => {
...
export const connect = (port, uri, reloadFullPage) => {
  if (FuseBox.isServer) {
    return;
  }
  ...
  function onSourceChanged(data, secondaryRequest) {
    ...
    if (data.type && data.type === "js" && data.dependencies) {
      ...
      if (req.files.length) {
        if (!secondaryRequest) {
          log(
            "Sending request for missing dependencies " +
              req.files.map(i => `${i.module}/${i.path}`),
          );
          ...
          log(
            "Error occured while updating dependency with list " +
              req.files.map(i => `${i.module}/${i.path}`),
          );
        }
      }
  ...
  client.on("bundle-error", ({ bundleName, message }) => {
  ...
  client.on("update-bundle-errors", ({ bundleName, messages }) => {
