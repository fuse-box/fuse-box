...
class Log {
    ...
    printOptions(title, obj) {
        ...
        log.addPreset('min', instance => {
            instance.formatter(data => {
                return log.inspector()(data).split('\n').map(data2 => indent2 + data2).map(data2 => data2.replace(/[{},]/, '')).join('\n');
            });
        });
    ...
    echoSparkyTaskHelp(taskName, taskHelp) {
        log.ansi().write(' ').cyan(taskName).white(taskHelp).echo();
    }
    groupHeader(str) {
...
class File {
    ...
    handleTypescript() {
        ...
        if (result.sourceMapText && this.context.useSourceMaps) {
            ...
            result.outputText = result.outputText.replace(`//# sourceMappingURL=${ this.info.fuseBoxPath }.map`, `//# sourceMappingURL=${ this.context.bundle.name }.js.map`).replace('//# sourceMappingURL=module.js.map', '');
    ...
    resolve(name, root, rootEntryLimit, parent) {
        ...
        if (data.isNodeModule) {
            ...
            if (info.target) {
                ...
                if (absPath.alias) {
                    if (parent) {
                        parent.analysis.replaceAliases(new Set([{
                                from: name,
...
function matchesDefinedExpression(node, expressions) {
    ...
    if (node.type === 'IfStatement' && node.test && node.test.type === 'BinaryExpression' && node.test.left && (node.test.left.type === 'MemberExpression' || node.test.left.type === 'Identifier')) {
        ...
        const writeBundle = bundle => {
            return bundle.context.output.writeCurrent(bundle.generatedCode).then(output => {
                ...
                if (bundle.quantumBit) {
                    splitFileOptions.i[bundle.quantumBit.name] = [
                        output.relativePath,
                        bundle.quantumBit.entry.getID()
                    ];
                }
            });
        };
...
class RawPluginClass {
    ...
    isRefreshRequired(file) {
        ...
        if (bundle && bundle.lastChangedFile) {
            const lastFile = file.context.convertToFuseBoxPath(bundle.lastChangedFile);
            if (Utils_1.isStylesheetExtension(bundle.lastChangedFile)) {
                return lastFile === file.info.fuseBoxPath || file.context.getItem('HMR_FILE_REQUIRED', []).indexOf(file.info.fuseBoxPath) > -1 || !!file.subFiles.find(subFile => subFile.info.fuseBoxPath === bundle.lastChangedFile);
            }
        }
    }
    transform(file) {
...
class Sparky {
    ...
    static showHelp() {
        exports.log.echoPlain('').groupHeader('Usage').echoPlain(`  ${ process.argv[0] } [TASK] [OPTIONS...]`).echoPlain('').groupHeader('Available tasks');
...
class CSSModulesClass {
    ...
    transform(file) {
        ...
        return new Promise((resolve, reject) => {
            ...
            file.cssDependencies = cssDependencies;
            return postcss([_373b.s('postcss-modules')({
...
$bundles
        ...
        let jsTags = this.opts.emitBundles ? this.opts.emitBundles(bundlePaths) : bundlePaths.map(bundle => `<script ${ this.opts.async ? 'async' : '' } type="text/javascript" src="${ bundle }"></script>`).join('\n');
        let preloadTags;
        if (this.opts.pre) {
...
async function npmPublish(opts) {
    ...
    return new Promise((resolve, reject) => {
        const publish = child_process_1.spawn('npm', [
            'publish',
            '--tag',
            opts.tag
        ], {
