...
describe("GenericRequireTest", () => {
	...
	it("Should cache a module", () => {
		return createEnv({
			...
			expect(out1).toEqual({ counter: 1 });

			let out2 = result.project.FuseBox.import("./index");
			expect(out2).toEqual({ counter: 1 });
		});
	});
	...
	it("Should require a folder with tilde", () => {
		return createEnv({
			project: {
				files: {
					"index.js": `exports.foo = require("./foo")`,
			...
			const out = result.project.FuseBox.import("./index");
			expect(out).toEqual({ foo: { bar: 20 } });
		});
	});

	it("Should require a folder with tilde (without ext)", () => {
		return createEnv({
			project: {
				files: {
					"index.js": `exports.foo = require("./foo")`,
	...
	it("Should require a folder with tilde (with ext)", () => {
		return createEnv({
			project: {
				files: {
					"index.js": `exports.foo = require("./foo")`,
	...
	it("Should require a relative require statement (folder)", () => {
		return createEnv({
			project: {
				files: {
					"index.js": `exports.foo = require("./foo")`,
			...
			const out = result.project.FuseBox.import("./index");
			expect(out).toEqual({ foo: { bar: 30 } });
		});
	});

	it("Should require a relative require statement (file without ext)", () => {
		return createEnv({
			project: {
				files: {
					"index.js": `exports.foo = require("./foo")`,
	...
	it("Should require a relative require statement (file with ext)", () => {
		return createEnv({
			project: {
				files: {
					"index.js": `exports.foo = require("./foo")`,
