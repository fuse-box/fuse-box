...
describe("CodeSplittingFileIntegrityTest", () => {
	it("Should create 1 split bundle with 1 file", () => {
	...
	it("Should create 1 split bundle with 2 files", () => {
		return FuseTestEnv.create({
	...
	it("Should create 2 split bundles with 2 files", () => {
		return FuseTestEnv.create({
			//testFolder : "_current_test",
			project: {
				files: {
	...
	it("Should create 2 split bundles with 2 files and 1 shared in app", () => {
		return FuseTestEnv.create({
			//testFolder : "_current_test",
			project: {
				files: {
	...
	it("Should create 2 split bundles with 2 files and 1 shared is moved from app", () => {
		return FuseTestEnv.create({
			//testFolder : "_current_test",
			project: {
				files: {
	...
	it("Should move node_module 'lib_a' and 'lib_b' into home", () => {
		return FuseTestEnv.create({
			//testFolder : "_current_test",
			project: {
				files: {
	...
	it("Should leave node_module 'lib_a' and 'lib_b' move into home", () => {
		return FuseTestEnv.create({
			//testFolder : "_current_test",
			project: {
				files: {
	...
	it("Should lazy load node_module 'lib_b'", () => {
		return FuseTestEnv.create({
			//testFolder : "_current_test",
			project: {
				files: {
	...
	it("Should lazy load node_module 'lib_b' second time in a split bundle", () => {
		return FuseTestEnv.create({
			//testFolder : "_current_test",
			project: {
				files: {
	...
	it("Should not load node_module 'lib_b' because it was imported", () => {
		return FuseTestEnv.create({
			//testFolder : "_current_test",
			project: {
				files: {
	...
	it("Should check if server data is set correctly", () => {
		...
		return FuseTestEnv.create({
            ...
            `,
					data => {
						expect(data.response).toEqual(`home`);
					},
					bundleMappingScript,
				),
			);
	...
	it("Should ignore a file with nested references", () => {
		return FuseTestEnv.create({
			//     testFolder: "_current_test",
			project: {
		...
		return FuseTestEnv.create({
			...
			.then(test =>
				test.browser((window, env) => {
					...
					expect(files).toEqual(["api.js", "app.js", "19879a97.js"]);
				}),
			);
	});
});
