...
export async function moduleCompiler(props: IModuleCompilerProps) {
  ...
  const ast = props.ast || (await getAST(props));
  ...
  for (const item of result.requireStatementCollection) {
    if (!item.statement.arguments[0]) {
      ...
      if (typeof item.statement.arguments[0].value === 'string') {
        ...
        props.onError('Invalid import. Should recieve a string');
      }
    }
  }
  const promises = [];
...
async function getAST(props: IModuleCompilerProps): Promise<ASTNode> {
  let parser: ICodeParser;

  let contents = props.contents;
  if (!contents && props.absPath) {
    contents = await readFileAsync(props.absPath);
  }
  const {
  ...
  try {
    ...
    props.onError(`Error while parsing module ${props.absPath}\n\t' ${e.stack || e.message}`);
    ast = parseJavascript(``);
  }
  return ast;
}
