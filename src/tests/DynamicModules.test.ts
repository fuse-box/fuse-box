...
export class DynamicModulesTest {
	...
	"Dynamic module should have access to the scope using tilde (without ext)"() {
		return createEnv({
			...
			should(fuse.import("./hello/world.js")).deepEqual({ foo: "bar" });
		});
	}
	...
	"Dynamic module can be overridden"() {
		return createEnv({
			...
			should(fuse.import("./hello/world.js")).deepEqual({ yes: true });
		});
	}
	...
	"FuseBox.exists should return true on dynamic module"() {
		return createEnv({
			...
			should(fuse.exists("./hello/world.js")).equal(true);
		});
	}
	...
	"FuseBox.exists should not throw if module is not found"() {
		return createEnv({
			...
			should(fuse.exists("app.html")).equal(false);
		});
	}
	...
	"Should register a module in a different package"() {
		return createEnv({
			...
			should(fuse.import("foo/hello/world")).deepEqual({ hello: "world" });
		});
	}
}
