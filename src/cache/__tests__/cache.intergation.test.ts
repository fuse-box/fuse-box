...
describe('Cache intergation test', () => {
  ...
  for (const strategy of strategies) {
    ...
    describe(`[ ${strategy} ] Workspace simple manupulations`, () => {
    ...
    describe(`[ ${strategy} ] Workspace module manupulation`, () => {
      let workspace: ITestWorkspace;
      beforeAll(() => {
        workspace = createTestWorkspace({
          files: {
      ...
      it('should update package foo', async () => {
        workspace.setModuleFile(
          'foo/package.json',
          JSON.stringify({ main: 'index.ts', name: 'foo', version: '2.0.0' }),
        );
      ...
      it('should introduce a new package within', async () => {
        workspace.setModuleFile(
          'foo/package.json',
          JSON.stringify({ main: 'index.ts', name: 'foo', version: '4.0.0' }),
        );
        workspace.setModuleFile('foo/index.ts', `export const fooPackage = require("bar")`);

        workspace.setModuleFile(
          'bar/package.json',
          JSON.stringify({ main: 'index.ts', name: 'bar', version: '1.0.0' }),
        );
