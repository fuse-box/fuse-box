(function(global, factory) {
  ...
  function skipSingleLineComment$$1(parser) {
    while (parser.index < parser.length) {
      if (CharTypes[parser.currentCodePoint] & 512 || (parser.currentCodePoint ^ 8232) <= 1) {
        break;
      }
  ...
  function skipMultiLineComment$$1(parser) {
    while (parser.index < parser.length) {
      ...
      if (CharTypes[parser.currentCodePoint] & 512 || (parser.currentCodePoint ^ 8232) <= 1) {
        if (
          CharTypes[parser.currentCodePoint] & 524288 &&
  ...
  function isIdentifierStart(code) {
    return code <= 0x7f ? CharTypes[code] & 1 : (unicodeLookup[(code >>> 5) + 34816] >>> code) & 31 & 1;
  }
  function isIdentifierPart(code) {
    return code <= 0x7f ? CharTypes[code] & 2 : (unicodeLookup[(code >>> 5) + 0] >>> code) & 31 & 1;
  }
    ...
    while (parser.index < parser.length) {
      ...
      if (first <= 0x7e) {
        ...
        switch (token) {
          ...
          case 130:
            parser.flags |= 1;
            if (CharTypes[first] & 524288 && CharTypes[parser.source.charCodeAt(parser.index + 1)] & 1048576) {
              parser.index++;
            }
            parser.column = 0;
            parser.currentCodePoint = parser.source.charCodeAt(++parser.index);
          ...
          case 16842797:
            ...
            return 8455738;
          case 8456756:
            if (nextCodePoint(parser) !== 61) return 8456756;
            nextCodePoint(parser);
            return 4194342;
          case 8456755: {
            nextCodePoint(parser);
            if (parser.index >= parser.length) return 8456755;
            const next = parser.currentCodePoint;
            if (next === 61) {
              nextCodePoint(parser);
              return 4194340;
            }
            if (next !== 42) return 8456755;
            nextCodePoint(parser);
            if (parser.currentCodePoint !== 61) return 8457014;
            nextCodePoint(parser);
            return 4194337;
          }
          case 8455238:
            if (nextCodePoint(parser) !== 61) return 8455238;
            nextCodePoint(parser);
            return 4194343;
          case 25233711: {
            nextCodePoint(parser);
            if (parser.index >= parser.length) return 25233711;
          ...
          case 25233712: {
            nextCodePoint(parser);
            if (parser.index >= parser.length) return 25233712;
            const next = parser.currentCodePoint;
            if (next === 45) {
              nextCodePoint(parser);
              if ((context & 2048) === 0 && (isStartOfLine || parser.flags & 1) && parser.currentCodePoint === 62) {
                if ((context & 256) === 0) report(parser, 124);
                skipSingleLineComment$$1(parser);
                continue;
              }
              return 33619996;
          ...
          case 8455999:
            nextCodePoint(parser);
            if (parser.index >= parser.length) return 8455999;
          ...
          case -2143289315: {
            nextCodePoint(parser);
            if (parser.index >= parser.length) return -2143289315;
            const next = parser.currentCodePoint;
            if (next === 61) {
              nextCodePoint(parser);
              if (parser.currentCodePoint === 61) {
          ...
          case 8454981: {
            nextCodePoint(parser);
            if (parser.index >= parser.length) return 8454981;
            const next = parser.currentCodePoint;
            if (next === 124) {
              nextCodePoint(parser);
              return 8978744;
          ...
          case 8456000: {
            nextCodePoint(parser);
            if (parser.index >= parser.length) return 8456000;
            const next = parser.currentCodePoint;
            if (next === 61) {
              nextCodePoint(parser);
              return 8455998;
            }
            if (next !== 62) return 8456000;
            nextCodePoint(parser);
            if (parser.index < parser.length) {
              const next = parser.currentCodePoint;
              if (next === 62) {
          ...
          case 8455492: {
            nextCodePoint(parser);
            if (parser.index >= parser.length) return 8455492;
            const next = parser.currentCodePoint;
            if (next === 38) {
              nextCodePoint(parser);
              return 8978999;
          ...
          case 67108877:
            nextCodePoint(parser);
            if ((CharTypes[parser.currentCodePoint] & 1024) !== 0) return scanNumber$$1(parser, context, true);
  ...
  function consumeMultiUnitCodePoint(parser, hi) {
    if ((hi & 0xfc00) !== 55296) return false;
  ...
  function scanIdentifierSlowCase(parser, context, hasEscape, canBeKeyword) {
    ...
    if (canBeKeyword && (length >= 2 && length <= 11)) {
      ...
      if (keyword === undefined) return 208897;
      if (keyword === 241770) return keyword;
      if ((keyword & 36864) === 36864) {
        return context & 1024 && hasEscape ? 143479 : keyword;
      }
      return context & 1024 && (keyword === 268677192 || keyword === 176233) ? 143479 : 143478;
    }
    return 208897;
  }
  function scanPrivateName$$1(parser) {
    nextCodePoint(parser);
    if (
      (CharTypes[parser.currentCodePoint] & 1024) !== 0 ||
  ...
  function scanUnicodeEscapeValue$$1(parser) {
    ...
    if ((CharTypes[parser.currentCodePoint] & 4096) === 0) report(parser, 6);
    const c2 = parser.source.charCodeAt(parser.index + 1);
    if ((CharTypes[c2] & 4096) === 0) report(parser, 0);
    const c3 = parser.source.charCodeAt(parser.index + 2);
    if ((CharTypes[c3] & 4096) === 0) report(parser, 0);
    const c4 = parser.source.charCodeAt(parser.index + 3);
    if ((CharTypes[c4] & 4096) === 0) report(parser, 0);
    ...
    while ((CharTypes[ch] & 512) === 0) {
      ...
      if ((ch & 8) === 8 && ch === 92) {
        ...
        if (ch > 0x7e) {
          ...
          else handleStringError(parser, code);
        }
        marker = parser.index + 1;
      }
      if (parser.index >= parser.length) report(parser, 14);
  ...
  function parseEscape(parser, context, first) {
    switch (first) {
      ...
      case 51: {
        ...
        if (index < parser.length) {
          ...
          if ((CharTypes[next] & 2048) === 0) {
            if ((code !== 0 || CharTypes[next] & 262144) && context & 1024) return -2;
      ...
      case 120: {
        ...
        if ((CharTypes[ch1] & 4096) === 0) return -4;
        const hi = toHex(ch1);
        const ch2 = nextCodePoint(parser);
        if ((CharTypes[ch2] & 4096) === 0) return -4;
        const lo = toHex(ch2);
        return (hi << 4) | lo;
      }
      case 117: {
        ...
        if (parser.currentCodePoint === 123) {
          ...
          if ((CharTypes[ch] & 4096) === 0) return -4;
          const c2 = parser.source.charCodeAt(parser.index + 1);
          if ((CharTypes[c2] & 4096) === 0) return -4;
          const c3 = parser.source.charCodeAt(parser.index + 2);
          if ((CharTypes[c3] & 4096) === 0) return -4;
          const c4 = parser.source.charCodeAt(parser.index + 3);
          if ((CharTypes[c4] & 4096) === 0) return -4;
          parser.index += 3;
          parser.column += 3;
  ...
  function scanNumber$$1(parser, context, isFloat) {
      ...
      if (kind & (16 | 32)) {
        if (atStart) {
          ...
          if (digit >= 0 && parser.currentCodePoint !== 46 && !isIdentifierStart(parser.currentCodePoint)) {
            if (context & 512) parser.tokenRaw = parser.source.slice(parser.startIndex, parser.index);
    ...
    let isBigInt = false;
    if (parser.currentCodePoint === 110 && (kind & (16 | 2 | 4 | 8)) !== 0) {
      if (isFloat) report(parser, 11);
      isBigInt = true;
      nextCodePoint(parser);
    ...
    if (context & 512) parser.tokenRaw = parser.source.slice(parser.tokenValue, parser.index);
  ...
  function scanTemplate(parser, context) {
    ...
    while (ch !== 96) {
      ...
      if (parser.index >= parser.length) report(parser, 15);
      ch = nextCodePoint(parser);
    }
    nextCodePoint(parser);
    parser.tokenValue = ret;
  ...
  function scanTemplateTail(state, context) {
    if (state.index >= state.length) return 129;
    state.index--;
  ...
  function scanRegularExpression$$1(parser, context) {
    ...
    loop: while (parser.index < parser.source.length) {
      switch (parser.currentCodePoint) {
        case 103:
          if (mask & 2) report(parser, 35, 'g');
          mask |= 2;
          break;
        case 105:
          if (mask & 1) report(parser, 35, 'i');
          mask |= 1;
          break;
        case 109:
          if (mask & 4) report(parser, 35, 'm');
          mask |= 4;
          break;
        case 117:
          if (mask & 16) report(parser, 35, 'g');
          mask |= 16;
          break;
        case 121:
          if (mask & 8) report(parser, 35, 'y');
          mask |= 8;
          break;
        case 115:
          if (mask & 12) report(parser, 35, 's');
          mask |= 12;
          break;
        default:
  ...
  function consume(parser, context, t) {
    if (parser.token !== t) report(parser, 23, KeywordDescTable[t & 255]);
    nextToken$$1(parser, context);
  }
  function reinterpretToPattern(state, node) {
    switch (node.type) {
      ...
      case 'AssignmentExpression':
        ...
        if (node.operator !== '=') report(state, 74);
  ...
  function validateIdentifier(parser, context, type, token) {
    if ((token & 4096) !== 4096) return;
    if (token === 176233) {
      if (context & 1024) report(parser, 105);
    }
    if ((token & 20480) === 20480) {
      report(parser, 112);
    }
    if ((token & 36864) === 36864) {
      if (context & 1024) report(parser, 107);
    }
    if (token === 268677192) {
      if (type & (8 | 16)) report(parser, 110);
    }
    if (token === 209005) {
  ...
  function isStrictReservedWord(parser, context, t) {
    if (t === 209005) {
      if (context & (4194304 | 2048)) report(parser, 108);
      parser.destructible |= 128;
    }
    if (t === 241770 && context & 2097152) report(parser, 106, 'yield');
    return (t & 20480) === 20480 || (t & 36864) === 36864 || t == 143479;
  }
  function validateArrowBlockBody(parser) {
    ...
    if ((parser.token & 8454144) === 8454144 && (parser.flags & 1) === 0) report(parser, 139);
    if ((parser.token & 33619968) === 33619968) report(parser, 139);
  }
  ...
  function parseSource(source, options, context) {
    if (options != null) {
      if (options.module) context |= 2048;
      ...
      if (options.globalAwait) context |= 64;
  ...
  function parseStatementList(parser, context) {
    ...
    while (parser.token === 134283267) {
      if (parser.index - parser.startIndex < 13 && parser.tokenValue === 'use strict') context |= 1024;
  ...
  function parseImportDeclarationOrImport(parser, context) {
    nextToken$$1(parser, context);
    if (parser.token !== 67174411) return parseImportDeclaration(parser, context);
  ...
  function parseStatementListItem(parser, context) {
    switch (parser.token) {
      ...
      case 133:
        if (context & 2048) return parseDecorators(parser, context | 268435456);
      case 86093:
      ...
      case 86105:
        nextToken$$1(parser, context);
        if (parser.token !== 67174411) report(parser, 113, 'import');
        let expr = { type: 'Import' };
        expr = parseMemberOrUpdateExpression(parser, context, expr, 0);
        return parseExpressionStatement(parser, context, expr);
      case 143468:
  ...
  function parseStatement(parser, context, allowFuncDecl) {
    switch (parser.token) {
      ...
      case 86103:
        report(parser, context & 1024 ? 83 : (context & 256) === 0 ? 85 : 84);
      case 86093:
        report(parser, 86);
      case 241770:
        const { token } = parser;
        let expr = parseYieldExpressionOrIdentifier(parser, context);
        if (parser.token === -1073741806) expr = parseSequenceExpression(parser, context, expr);
        if (context & 2097152) return parseExpressionStatement(parser, context, expr);
        if (parser.token === 21) {
  ...
  function parseExpressionOrLabelledStatement(parser, context, allowFuncDecl) {
    ...
    switch (token) {
      case 268677192:
        ...
        if (context & 1024) report(parser, 93);
        if (parser.token === 21) return parseLabelledStatement(parser, context, expr, token, allowFuncDecl);
  ...
  function parseReturnStatement(parser, context) {
    if ((context & 32) === 0 && context & 8192) report(parser, 100);
    nextToken$$1(parser, context | 32768);
    const argument = parser.flags & 1 || parser.token & 1048576 ? null : parseExpressions(parser, context, 1);
  ...
  function parseLabelledStatement(parser, context, expr, token, allowFuncDecl) {
    if ((token & 20480) === 20480) report(parser, 0);
    nextToken$$1(parser, context | 32768);
    return {
      type: 'LabeledStatement',
      label: expr,
      body:
        allowFuncDecl && (context & 1024) === 0 && context & 256 && parser.token === 86103
  ...
  function parseAsyncArrowOrAsyncFunctionDeclaration(parser, context, allowFuncDecl) {
    ...
    if (parser.token === 21) return parseLabelledStatement(parser, context, expr, token, 1);
    const asyncNewLine = parser.flags & 1;
    if (!asyncNewLine) {
      if (parser.token === 86103) {
        if (!allowFuncDecl) report(parser, 137);
        return parseFunctionDeclaration(parser, context, 0, 1);
      }
      if ((parser.token & 143360) === 143360) {
        if (parser.assignable & 2) report(parser, 48);
        if (parser.token === 209005) report(parser, 30);
        if (context & (1024 | 2097152) && parser.token === 241770) report(parser, 30);
        if ((parser.token & 537079808) === 537079808) parser.flags |= 128;
    ...
    if (parser.token === 67174411)
      expr = parseAsyncArrowOrCallExpression(parser, context & ~134217728, expr, 1, asyncNewLine);
    else if (parser.token === 10) expr = parseArrowFunctionExpression(parser, context, [expr], 0);
    else {
      parser.assignable = 1;
    }
    expr = parseMemberOrUpdateExpression(parser, context, expr, 0);
  ...
  function parseDirective(parser, context) {
    if ((context & 8) < 1) return parseStatementListItem(parser, context);
  ...
  function parseThrowStatement(parser, context) {
    nextToken$$1(parser, context | 32768);
    if (parser.flags & 1) report(parser, 98);
  ...
  function parseIfStatement(parser, context) {
    ...
    const alternate = consumeOpt(parser, context | 32768, 20562) ? parseConsequentOrAlternate(parser, context) : null;
    return {
      type: 'IfStatement',
      test,
      consequent,
  ...
  function parseConsequentOrAlternate(parser, context) {
    return context & 1024 || (context & 256) === 0 || parser.token !== 86103
  ...
  function parseSwitchStatement(parser, context) {
    ...
    while (parser.token !== -2146435057) {
      ...
      if (consumeOpt(parser, context | 32768, 20555)) {
        ...
        if (seenDefault) report(parser, 97);
        seenDefault = 1;
      }
      consume(parser, context | 32768, 21);
      while (parser.token !== 20555 && parser.token !== -2146435057 && parser.token !== 20560) {
  ...
  function parseContinueStatement(parser, context) {
    if ((context & 131072) === 0) report(parser, 69);
    nextToken$$1(parser, context);
    let label = null;
  ...
  function parseWithStatement(parser, context) {
    nextToken$$1(parser, context);
    if (context & 1024) report(parser, 99);
    consume(parser, context | 32768, 67174411);
  ...
  function parseTryStatement(parser, context) {
    ...
    const handler = consumeOpt(parser, context | 32768, 20556) ? parseCatchBlock(parser, context) : null;
    const finalizer = consumeOpt(parser, context | 32768, 20565) ? parseBlock(parser, context) : null;
  ...
  function parseVariableDeclaration(parser, context, type, origin) {
    ...
    if (parser.token === -2143289315) {
      ...
      if (origin & 4 || (token & 2097152) === 0) {
        if (
          parser.token === 274546 ||
          (parser.token === 8738609 && (token & 2097152 || (type & 4) === 0 || (context & 256) === 0 || context & 1024))
        ) {
          report(parser, 61, parser.token === 274546 ? 'of' : 'in');
        }
      }
    } else if ((type & 16 || (token & 2097152) > 0) && (parser.token & 262144) !== 262144) {
  ...
  function parseForStatement(parser, context) {
      ...
      if (token === 268677192) {
        ...
        if (parser.token & (143360 | 2097152)) {
          if (parser.token === 8738609) {
            if (context & 1024) report(parser, 68);
          } else {
            init = {
              type: 'VariableDeclaration',
              kind: 'let',
          ...
          if (parser.token === 274546) report(parser, 127);
        }
      } else {
        nextToken$$1(parser, context);
        init = {
    ...
    if ((parser.token & 262144) === 262144) {
      ...
      if (!isOf && forAwait) report(parser, 90);
      reinterpretToPattern(parser, init);
      nextToken$$1(parser, context | 32768);
      right = isOf ? parseExpression(parser, context, 1) : parseExpressions(parser, context, 1);
      consume(parser, context | 32768, 1073741840);
      const body = parseStatement(parser, ((context | 16384) ^ 16384) | 131072, 0);
      return isOf
        ? {
    ...
    if (!isVarDecl) {
      ...
      if (destructible & 8) report(parser, 136);
    }
    if (parser.token === -1073741806) init = parseSequenceExpression(parser, context, init);
    consume(parser, context | 32768, -2146435055);
    if (parser.token !== -2146435055) test = parseExpressions(parser, context, 1);
    consume(parser, context | 32768, -2146435055);
    if (parser.token !== 1073741840) update = parseExpressions(parser, context, 1);
    consume(parser, context | 32768, 1073741840);
    const body = parseStatement(parser, ((context | 16384) ^ 16384) | 131072, 0);
    return {
      type: 'ForStatement',
  ...
  function parseImportDeclaration(parser, context) {
    ...
    if (parser.token === 134283267) {
      ...
      if (parser.token & 143360) {
        ...
        parseImportSpecifierOrNamedImports(parser, context, specifiers);
      } else report(parser, 29, KeywordDescTable[parser.token & 255]);
  ...
  function parseModuleSpecifier(parser, context) {
    ...
    if (parser.token !== 134283267) report(parser, 117, 'Import');
  ...
  function parseImportSpecifierOrNamedImports(parser, context, specifiers) {
    ...
    while (parser.token & 143360) {
      ...
      if (parser.token !== -2146435057) consume(parser, context, -1073741806);
    }
  ...
  function parseExportDeclaration(parser, context) {
    ...
    switch (parser.token) {
      case 8456755: {
        ...
        if (parser.token !== 134283267) report(parser, 117, 'Export');
        source = parseLiteral(parser, context);
        consumeSemicolon(parser, context | 32768);
  ...
  function parseAssignmentExpression(parser, context, left) {
    if ((parser.token & 4194304) > 0) {
      ...
      if ((parser.token === -2143289315 && left.type === 'ArrayExpression') || left.type === 'ObjectExpression') {
  ...
  function parseBinaryExpression(parser, context, minPrec, left) {
    ...
    while (parser.token & 8454144) {
      t = parser.token;
      prec = t & 3840;
      if (prec + ((t === 8457014) << 8) - ((bit === t) << 12) <= minPrec) break;
      nextToken$$1(parser, context | 32768);
      left = {
    ...
    if (parser.token === -2143289315) report(parser, 24);
    return left;
  }
  function parseUnaryExpression(parser, context) {
    ...
    if (parser.token === 8457014) report(parser, 32);
  ...
  function parseYieldExpressionOrIdentifier(parser, context) {
    ...
    if (context & 2097152) {
      nextToken$$1(parser, context | 32768);
      if (context & 8388608) report(parser, 31);
      if (parser.token === 22) report(parser, 138);
      let argument = null;
      let delegate = false;
      if ((parser.flags & 1) < 1) {
    ...
    if (context & 1024) report(parser, 123, 'Yield');
  ...
  function parseAwaitExpressionOrIdentifier(parser, context, inNewExpression) {
    ...
    if (context & 2048) report(parser, 123, 'Await');
  ...
  function parseFunctionBody(parser, context, origin, firstRestricted) {
    ...
    if (parser.token !== -2146435057) {
      ...
      if (
        context & 1024 &&
        firstRestricted &&
        ((firstRestricted & 537079808) === 537079808 || (firstRestricted & 36864) === 36864)
      ) {
        report(parser, 39);
      }
    }
    ...
    if (parser.token === -2143289315) report(parser, 133);
    return {
      type: 'BlockStatement',
      body,
    };
  ...
  function parseSuperExpression(parser, context) {
    nextToken$$1(parser, context);
    if (context & 536870912) report(parser, 29, 'super');
    switch (parser.token) {
      case 67174411: {
        if ((context & 524288) === 0) report(parser, 27);
        parser.assignable = 2;
        break;
      }
      case 69271571:
      case 67108877: {
        if ((context & 262144) === 0) report(parser, 28);
        parser.assignable = 1;
        break;
      }
      default:
  ...
  function parseMemberOrUpdateExpression(parser, context, expr, inNewExpression) {
    if ((parser.token & 33619968) === 33619968 && (parser.flags & 1) === 0) {
      if (parser.assignable & 2) report(parser, 56);
    ...
    if ((parser.token & 67108864) === 67108864) {
      if (parser.token === 67108877) {
        nextToken$$1(parser, context);
        if ((parser.token & (143360 | 4096)) === 0 && parser.token !== 131) report(parser, 0);
        parser.assignable = 1;
        expr = {
          ...
          computed: false,
          property:
            context & 1 && parser.token === 131 ? parsePrivateName(parser, context) : parseIdentifier(parser, context),
        };
        ...
        const args = parseArguments(parser, context & ~134217728);
        parser.assignable = 2;
        expr = {
          type: 'CallExpression',
          callee: expr,
        ...
        expr = {
          ...
          tag: expr,
          quasi: parser.token === 67174408 ? parseTemplate(parser, context) : parseTemplateLiteral(parser, context),
        };
      }
      return parseMemberOrUpdateExpression(parser, context, expr, inNewExpression);
    } else if (inNewExpression) {
  ...
  function parsePrimaryExpressionExtended(parser, context, type, inNewExpression, assignable) {
    ...
    if ((token & 33619968) === 33619968) {
      if (inNewExpression) report(parser, 57);
      const updateToken = parser.token;
      nextToken$$1(parser, context | 32768);
      const arg = parseLeftHandSideExpression(parser, context, 0);
      if (parser.assignable & 2) {
        report(parser, (parser.token & 537079808) === 537079808 ? 126 : 56);
      }
      parser.assignable = 2;
      return {
        type: 'UpdateExpression',
    ...
    if (token === 241770) {
      ...
      if (context & ((context & 2097152) | 1024)) report(parser, 106, 'yield');
      return parseIdentifier(parser, context);
    }
    if (parser.token === 268677192) {
      if (context & 1024) report(parser, 125);
      if (type & (8 | 16)) report(parser, 65);
    }
    if ((token & 143360) === 143360) {
      ...
      if (token === 143478) report(parser, 103);
      const IsEvalOrArguments = (token & 537079808) === 537079808;
      if (parser.token === 10) {
        if (IsEvalOrArguments) {
          if (context & 1024) report(parser, 131);
          parser.flags |= 128;
        } else parser.flags = (parser.flags | 128) ^ 128;
        if (!assignable) report(parser, 58);
        return parseArrowFunctionExpression(parser, context, [expr], 0);
      }
      parser.assignable = context & 1024 && IsEvalOrArguments ? (parser.assignable = 2) : 1;
      return expr;
    }
      ...
      default:
        if (
          context & 1024
            ...
            : (token & 143360) === 143360 || (token & 12288) === 12288 || (token & 36864) === 36864
        ) {
          parser.assignable = 1;
  ...
  function parseCallImportOrMetaProperty(parser, context, isNew) {
    ...
    if (consumeOpt(parser, context, 67108877)) {
      if (context & 2048 && parser.tokenValue === 'meta') return parseMetaProperty(parser, context, id);
      report(parser, 29, KeywordDescTable[parser.token & 255]);
    } else if ((isNew && parser.token === 67174411) || parser.token === -2146435057 || parser.token === 20) report(parser, 29, KeywordDescTable[parser.token & 255]);
    return {
      type: 'Import',
    };
  }
  ...
  function parseArguments(parser, context) {
    ...
    while (parser.token !== 1073741840) {
      if (parser.token === 241770) parser.destructible |= 256;
      args.push(parser.token === 14 ? parseSpreadElement(parser, context) : parseExpression(parser, context, 1));
      if (parser.token !== -1073741806) break;
      consume(parser, context | 32768, -1073741806);
      if (parser.token === 1073741840) break;
    }
    consume(parser, context, 1073741840);
    return args;
  }
  ...
  function parseFunctionDeclaration(parser, context, flags, isAsync) {
    ...
    context = ((context | 32243712) ^ 32243712) | 67108864 | ((isAsync * 2 + isGenerator) << 21);
    return {
      type: 'FunctionDeclaration',
      params: parseFormalParametersOrFormalList(parser, context | 8388608, 1),
      body: parseFunctionBody(
        parser,
        (context | 16384 | 8192 | 135168 | 536870912) ^ (8192 | 16384 | 135168 | 536870912),
        1,
        firstRestricted,
      ),
      async: isAsync === 1,
  ...
  function parseFunctionExpression(parser, context, isAsync) {
    ...
    const body = parseFunctionBody(
      parser,
      (context | 8192 | 16384 | 135168 | 536870912) ^ (8192 | 16384 | 135168 | 536870912),
      0,
      firstRestricted,
    );
    parser.assignable = 2;
  ...
  function parseArrayExpressionOrPattern(parser, context, skipInitializer, type) {
      ...
      if (consumeOpt(parser, context | 32768, -1073741806)) {
        ...
        if (token & 143360) {
          ...
          if (consumeOpt(parser, context | 32768, -2143289315)) {
            ...
            destructible |= (parser.token === 209005 ? 128 : 0) | (parser.token === 241770 ? 256 : 0);
            left = {
              type: 'AssignmentExpression',
              operator: '=',
              left,
            ...
            destructible |=
              parser.assignable & 2 ? 16 : 0 | (token === 209005 ? 128 : 0) | (token === 241770 ? 256 : 0);
          } else {
            if (type) destructible |= 16;
            ...
            if (parser.token !== -1073741806 && parser.token !== 20) {
              if (parser.token !== -2143289315) destructible |= 16;
              left = parseAssignmentExpression(parser, context, left);
            } else if (parser.token !== -2143289315) {
              destructible |= type || parser.assignable & 2 ? 16 : 32;
            }
          }
          ...
          if (parser.token !== -1073741806 && parser.token !== 20) {
            left = parseAssignmentExpression(parser, context, left);
            if (type && token === 67174411) destructible |= 16;
          } else if (parser.assignable & 2) {
            destructible |= 16;
          } else if (token === 67174411) {
            destructible |= parser.assignable & 1 && !type ? 32 : token === 67174411 || parser.assignable & 2 ? 16 : 0;
          }
        }
        elements.push(left);
        if (consumeOpt(parser, context | 32768, -1073741806)) {
    ...
    if (!skipInitializer) {
      ...
      if (parser.token & 4194304) report(parser, 72);
    }
  ...
  function parseArrayOrObjectAssignmentPattern(parser, context, destructible, node) {
    ...
    parser.destructible =
      ((destructible | 64 | 8) ^ (8 | 64)) | (parser.flags & 256 ? 128 : 0) | (token === 241770 ? 256 : 0);
    return {
      type: 'AssignmentExpression',
      left: node,
      operator: '=',
  ...
  function parseRestOrSpreadElement(parser, context, closingToken, type, isAsync) {
    ...
    if (parser.token & (4096 | 143360)) {
      ...
      if (parser.token !== -1073741806 && parser.token !== closingToken) {
        if (parser.assignable & 2 && parser.token === -2143289315) report(parser, 74);
        destructible |= 16;
        argument = parseAssignmentExpression(parser, context, argument);
      }
      destructible |= parser.assignable & 2 ? 16 : token !== closingToken && token !== -1073741806 ? 32 : 0;
      ...
      if (token !== -2143289315 && token !== closingToken && token !== -1073741806) {
        if (parser.destructible & 8) report(parser, 74);
        argument = parseMemberOrUpdateExpression(parser, context, argument, 0);
        destructible |= parser.assignable & 2 ? 16 : 0;
        const { token } = parser;
        if (parser.token !== -1073741806 && parser.token !== closingToken) {
          argument = parseAssignmentExpression(parser, context, argument);
          if (token !== -2143289315) destructible |= 16;
        } else if (token !== -2143289315) {
          destructible |= type || parser.assignable & 2 ? 16 : 32;
        }
      } else {
        destructible |= closingToken === -2146435057 && token !== -2143289315 ? 16 : parser.destructible;
      }
    } else {
      if (type) report(parser, 41);
      argument = parseLeftHandSideExpression(parser, context, 1);
      const { token } = parser;
      if (token === -2143289315 && token !== closingToken && token !== -1073741806) {
        if (parser.assignable & 2) report(parser, 41);
        argument = parseAssignmentExpression(parser, context, argument);
        destructible |= 16;
      } else {
        if (token === -1073741806) {
    ...
    if (parser.token !== closingToken) {
      if (!isAsync && type & 1) {
        report(parser, parser.token === -1073741806 ? 59 : parser.token === -2143289315 ? 76 : 78);
      }
      if (consumeOpt(parser, context | 32768, -2143289315)) {
  ...
  function parseMethodDefinition(parser, context, kind) {
    ...
    context = ((context | kindFlags) ^ kindFlags) | ((kind & 88) << 18) | 100925440;
    return {
      type: 'FunctionExpression',
      params: parseMethodFormals(parser, context | 8388608, kind, 1),
      body: parseFunctionBody(parser, (context | 8192 | 16384 | 135168) ^ (8192 | 16384 | 135168), 0, void 0),
      async: (kind & 16) > 0,
      generator: (kind & 8) > 0,
      id: null,
    };
      ...
      if (parser.token === 14) {
        ...
        if (parser.token & (143360 | (parser.token & 4096))) {
          ...
          if (parser.token === -1073741806 || parser.token === -2146435057 || parser.token === -2143289315) {
            state |= 4;
            if ((token & 537079808) === 537079808) {
              if (context & 1024) destructible |= 16;
            } else {
              validateIdentifier(parser, context, type, token);
            }
            destructible |= token === 209005 ? 128 : 0;
            if (consumeOpt(parser, context | 32768, -2143289315)) {
              destructible |= 8 | (parser.token === 209005 ? 128 : 0) | (parser.token === 241770 ? 256 : 0);
              value = {
                type: 'AssignmentPattern',
                left: key,
                right: parseExpression(parser, (context | 134217728) ^ 134217728, 1),
              };
            } else {
              value = key;
            }
            ...
            if (parser.token & 143360) {
              ...
              if (parser.token === -1073741806 || parser.token === -2146435057) {
                ...
                destructible |= parser.assignable & 2 ? 16 : token === -2143289315 ? 0 : 32;
              ...
              if (parser.token === -1073741806 || parser.token === -2146435057) {
                ...
                value = parseMemberOrUpdateExpression(parser, context, value, 0);
                destructible = parser.assignable & 1 ? 0 : destructible | 16;
                const { token } = parser;
                if (parser.token !== -1073741806 && parser.token !== -2146435057) {
                  value = parseAssignmentExpression(parser, (context | 134217728) ^ 134217728, value);
                  if (token !== -2143289315) destructible |= 16;
            ...
            destructible |= 16;
            if (token === 143468) state |= 16;
            if (token === 12399) state |= 256;
            else if (token === 12400) state |= 512;
            else state |= 1;
            ...
            destructible |= 16;
            if (token === 143468) {
              if (parser.flags & 1) report(parser, 0);
              state |= 16;
            }
            key = parseIdentifier(parser, context);
            if (token === 143478) report(parser, 0);
            if (token === 12399) state |= 256;
            else if (token === 12400) state |= 512;
            else state |= 1;
            value = parseMethodDefinition(parser, context, state);
            ...
            destructible |= 16;
            if (token === 143478) report(parser, 22);
          ...
          if (parser.token === 21) {
            ...
            if (parser.token & 143360) {
              ...
              if (parser.token === -1073741806 || parser.token === -2146435057) {
                ...
                if (parser.token !== -1073741806 && parser.token !== -2146435057) {
                  if (notAssignable) destructible |= 16;
                  value = parseAssignmentExpression(parser, (context | 134217728) ^ 134217728, value);
                  if (notAssignable) destructible |= 16;
          ...
          if (parser.token & 143360) {
            ...
            if (parser.token === 67174411) {
              ...
              reportAt(
                ...
                line,
                index,
                token === 143468 ? 45 : token === 12399 || parser.token === 12400 ? 44 : 46,
    ...
    if (kind & 256) {
      ...
      while (parser.token !== 1073741840) {
        ...
        if (parser.token & 143360) {
          if (
            (context & 1024) === 0 &&
            ((parser.token & 36864) === 36864 || (parser.token & 537079808) === 537079808)
          ) {
            isComplex = 1;
          }
          ...
          isComplex = 1;
          reinterpretToPattern(parser, left);
          if (parser.destructible & 16) report(parser, 51);
          if (type && parser.destructible & 32) report(parser, 51);
        }
        if (parser.token === -2143289315) {
          nextToken$$1(parser, context | 32768);
          isComplex = 1;
        ...
        setterArgs++;
        params.push(left);
        if (parser.token !== 1073741840) consume(parser, context, -1073741806);
      }
      if (isComplex) parser.flags |= 128;
  ...
  function parseParenthesizedExpression(parser, context, assignable) {
    ...
    while (parser.token !== 1073741840) {
      if (parser.token & (143360 | 4096)) {
        const { token } = parser;
        if ((token & 537079808) === 537079808 || (token & 36864) === 36864) {
          isComplex = 1;
        }
        expr = parsePrimaryExpressionExtended(parser, context, 0, 0, 1);
        if (consumeOpt(parser, context | 32768, -2143289315)) {
          ...
          destructible |= (parser.token & 1073741824) === 1073741824 ? (parser.assignable & 2 ? 16 : 0) : 16;
          expr = parseMemberOrUpdateExpression(parser, context, expr, 0);
          expr = parseAssignmentExpression(parser, context, expr);
        }
      } else if (parser.token & 2097152) {
        ...
        if ((parser.token & 1073741824) !== 1073741824) {
          if (destructible & 8) report(parser, 135);
          expr = parseMemberOrUpdateExpression(parser, context, expr, 0);
          destructible |= 16;
          if ((parser.token & 1073741824) !== 1073741824) {
            expr = parseAssignmentExpression(parser, context, expr);
        ...
        if (parser.destructible & 16) report(parser, 79);
        isComplex = 1;
        destructible |= parser.destructible;
      ...
      if (!consumeOpt(parser, context | 32768, -1073741806)) break;
      if (!toplevelComma) {
        toplevelComma = 1;
        expressions = [expr];
      }
    ...
    if (destructible & 16 && destructible & 8) report(parser, 0);
    if (parser.token === 10) {
      if (isComplex) parser.flags |= 128;
      if (!assignable) report(parser, 49);
      if (destructible & 16) report(parser, 49);
      if (destructible & 32) report(parser, 50);
      if (context & (4194304 | 2048) && parser.destructible & 128) report(parser, 30);
  ...
  function parseArrowFunctionExpression(parser, context, params, isAsync) {
    if (parser.flags & 1) report(parser, 47);
    ...
    if (expression) {
      ...
      body = parseFunctionBody(
        parser,
        (context | 8192 | 16384 | 135168 | 134217728) ^ (8192 | 16384 | 135168 | 134217728),
        2,
        void 0,
      );
  ...
  function parseFormalParametersOrFormalList(parser, context, type) {
    ...
    while (parser.token !== 1073741840) {
      let left;
      if (parser.token & 143360) {
        if ((context & 1024) === 0 && ((parser.token & 36864) === 36864 || (parser.token & 537079808) === 537079808)) {
          isComplex = 1;
        }
        left = parseAndClassifyIdentifier(parser, context, type);
      } else {
  ...
  function parseNewExpression(parser, context) {
    ...
    return {
      type: 'NewExpression',
      callee,
      arguments: parser.token === 67174411 ? parseArguments(parser, context & ~134217728) : [],
    };
  }
  ...
  function parseAsyncExpression(parser, context, token, expr, inNewExpression, assignable) {
    ...
    if (!isNewLine) {
      if (token === 143478) report(parser, 103);
      if (parser.token === 86103) return parseFunctionExpression(parser, context, 1);
      if ((parser.token & 143360) === 143360) {
        if (parser.assignable & 2) report(parser, 48);
        if (parser.token === 209005) report(parser, 30);
    ...
    if (parser.token === 10) {
      if (inNewExpression) report(parser, 52);
      return parseArrowFunctionExpression(parser, context, [expr], 0);
    }
    return expr;
  }
  function parseAsyncArrowOrCallExpression(parser, context, callee, assignable, asyncNewLine) {
    ...
    if (consumeOpt(parser, context, 1073741840)) {
      if (parser.token === 10) {
        if (asyncNewLine) report(parser, 47);
        if (!assignable) report(parser, 48);
    ...
    while (parser.token !== 1073741840) {
      if (parser.token & (143360 | 4096)) {
        if ((parser.token & 537079808) === 537079808 || (parser.token & 36864) === 36864) {
          isComplex = 1;
        }
        ...
        if (consumeOpt(parser, context | 32768, -2143289315)) {
          ...
          const right = parseExpression(parser, context, 1);
          parser.assignable = 2;
          parser.destructible |= parser.flags & 256 ? 128 : 0 | (parser.flags & 512) ? 256 : 0;
          expr = {
            type: 'AssignmentExpression',
            left: expr,
            operator: '=',
        ...
        if ((parser.token & 1073741824) !== 1073741824) {
          ...
          expr = parseMemberOrUpdateExpression(parser, context, expr, 0);
          destructible |= 16;
          if ((parser.token & 1073741824) !== 1073741824) expr = parseAssignmentExpression(parser, context, expr);
        }
      } else if (parser.token === 14) {
        expr = parseRestOrSpreadElement(parser, context, 1073741840, 1, 1);
        destructible |= parser.destructible;
        isComplex = 1;
        if (parser.token !== 1073741840) parser.destructible |= 16;
      } else {
        expr = parseExpression(parser, context, 1);
        destructible = parser.assignable;
        params.push(expr);
    ...
    if (parser.token === 10) {
      if (isComplex) parser.flags |= 128;
      if (!assignable) report(parser, 49);
      if (destructible & 16) report(parser, 25);
      if (destructible & 32) report(parser, 50);
      if (parser.flags & 1 || asyncNewLine) report(parser, 47);
      if (parser.destructible & 128) report(parser, 31);
      if (context & (1024 | 2097152) && parser.destructible & 256) report(parser, 31);
  ...
  function parseClassDeclaration(parser, context, requireIdentifier) {
    ...
    nextToken$$1(parser, context);
    if (((parser.token & 4351) ^ 84) > 4096) {
      if (isStrictReservedWord(parser, context, parser.token)) report(parser, 130);
      if ((parser.token & 537079808) === 537079808) report(parser, 131);
      id = parseIdentifier(parser, context);
  ...
  function parseClassBody(parser, context, type, origin, decorators) {
    ...
    while (parser.token !== -2146435057) {
      if (context & 1) {
        ...
        if (parser.token === -2146435057) report(parser, 121);
        if (consumeOpt(parser, context, -2146435055)) {
          if (decorators.length > 0) report(parser, 132);
          continue;
        }
      }
      if (consumeOpt(parser, context, -2146435055)) continue;
  ...
  function parseClassElementList(parser, context, type, decorators, isStatic) {
    ...
    if (kind & (8 | 16 | 768)) {
      if (kind & 8 && parser.token === 67174411) report(parser, 0);
      if (parser.token & 143360) {
        key = parseIdentifier(parser, context);
      } else if ((parser.token & 134217728) === 134217728) {
        key = parseLiteral(parser, context);
      ...
      if (parser.token !== 67174411) return parseFieldDefinition(parser, context, key, kind, decorators);
    } else if (kind & 128) {
      context = context | 536870912;
      if (parser.token !== 67174411) return parseFieldDefinition(parser, context, key, kind, decorators);
    }
    if (parser.tokenValue === 'constructor') {
      if ((kind & 32) === 0) {
        if ((kind & 2) === 0 && kind & (768 | 16 | 128 | 8)) {
          report(parser, 54, 'accessor');
        }
        if ((context & 524288) === 0 && (kind & 2) === 0) {
          if (parser.flags & 32) report(parser, 55);
          else parser.flags |= 32;
        }
      }
      kind |= 64;
    }
    if ((kind & 2) === 0 && kind & (32 | 8 | 16 | 768) && parser.tokenValue === 'prototype') {
      report(parser, 53);
    }
    if (context & 1 && parser.token !== 67174411) {
      if (parser.tokenValue === 'constructor') report(parser, 143);
      return parseFieldDefinition(parser, context, key, kind, decorators);
    }
  ...
  function parsePrivateName(parser, context) {
    ...
    if (name === 'constructor') report(parser, 142);
    nextToken$$1(parser, context);
    return {
      type: 'PrivateName',
      name,
  ...
  function parseFieldDefinition(parser, context, key, state, decorators) {
    let value = null;
    if (state & 32 && parser.tokenValue === 'constructor') report(parser, 0);
  ...
  function parseBindingPattern(parser, context, type) {
    if (parser.token & 143360) return parseAndClassifyIdentifier(parser, context, type);
    if ((parser.token & 2097152) !== 2097152) report(parser, 29, KeywordDescTable[parser.token & 255]);
    const left =
      parser.token === 69271571
        ? parseArrayExpressionOrPattern(parser, context, 1, type)
        : parseObjectLiteralOrPattern(parser, context, 1, type);
  ...
  function parseAndClassifyIdentifier(parser, context, type) {
    ...
    if (token === 268677192) {
      if (type & 32) report(parser, 111);
      if (type & (8 | 16)) report(parser, 110);
      if (context & 1024) report(parser, 109);
    }
