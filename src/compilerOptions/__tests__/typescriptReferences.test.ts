...
describe('test case 3', () => {
    ...
    const expectedGrouped = new Map<string, Map<string, string>>([
        ...
        [P(fuseRoot, case3, `/pkgA`), new Map<string, string>([
            [P(`dist/alpha.js`), P(`src/alpha.ts`)],
        ])],
        [P(fuseRoot, case3, `/pkgB`), new Map<string, string>([
            [P(`dist/beta.js`), P(`src/beta.ts`)],
        ])],
        [P(fuseRoot, case3, `/pkgC`), new Map<string, string>([
        ...
        [P(fuseRoot), new Map<string, string>([
    ...
    it('creates a working resolver from grouped mappings', () => {
        ...
        expect(resolver({ fileDir: P(fuseRoot, case3, `pkgC`), target: P(`output/charlie`) }))
            .toEqual({
        ...
        expect(resolver({ fileDir: P(fuseRoot, case3, `pkgC`), target: P(`output/charlie.js`) }))
            .toEqual({
                absPath: P(fuseRoot, case3, `pkgC/input/charlie.ts`),
                extension: ".ts",
                fileExists: true,
        ...
        expect(resolver({ fileDir: P(fuseRoot, case3, `pkgC`), target: `` }))
            .toEqual({
